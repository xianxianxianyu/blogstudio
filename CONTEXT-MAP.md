...
**根 (Root)**:
外壳左上角那几个：**Book**、**Context**、**Writer**、**Loop**、**Export**。
...
**发布**（2026-09-07 退休）：作为**根的名字**退休，那一根现在叫 **Export**——五个根里
只有它是中文，而它跟旁边四个是同一类东西。**代码里的 `publish` 不动**：那指的是
「翻译 → 构建 → 同步」那条管线（`blogstudio/src/publish/`、`/__publish`），
与那一列不是一样东西。根的 `RootKind` 已改成 `export`。
