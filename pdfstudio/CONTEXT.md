...
**标签 (Label)**:
摘录在页面上的可视形态：圆点 ⇄ 小窗。**只关乎显示大小，不关乎内容**。
_Avoid_: 标记（那是颜色分类）、批注、卡片

**标记 (Tag)**:
摘录的阅读态分类，**颜色即分类**——要点 / 读懂了 / 术语 / 存疑 / 待查。五色闭集，
一条摘录最多一个，名字的注册表在书架根上（改名不重写摘录文件）。
与 ☆ 重要是两回事：标记说「这是什么」，☆ 说「别删」。
_Avoid_: 标签（那是圆点⇄小窗的形态）、主题（那是知识库的词，见下）、高亮、分类

**目录 (Outline)**:
一本书的层级结构：一棵可导航、可编辑的小节树，摘录挂在它下面。**它是产物，长期使用。**
两个来源——PDF 文件自带的书签，或从目录页生成的那一份；生成的优先（读者会去生成，
正是因为自带的没有或不好用）。
_Avoid_: 目录页（那是素材）、书签（那只是自带目录的一种来源）、大纲

**目录页 (TOC pages)**:
书自己印着目录的那几页，是 PDF 里实实在在的物理页。**它是素材，一次性的输入**——认完
就退场，之后读者面对的只有目录。
_Avoid_: 目录（那是产物）

**小节 (Section)**:
目录里的一个条目：标题、它从哪一物理页开始、层级。**空的小节照样是小节**——它不因为
底下没有摘录就不存在（ADR-0018）。
_Avoid_: 章节（书里「章」和「节」是不同层级，这个词指任意一层）、条目

**印刷页码 (Printed page)** ⇄ **物理页码 (Physical page)**:
目录行右端印着的那个数字，与它在 PDF 里实际是第几页。两者相差一个**偏移 (Offset)**
——封面、版权页、前言都不计入印刷页码。**这一对不能混**：混了整本书的跳转会整体
歪掉，而且不报任何错。
_Avoid_: 页码（不指明是哪一个就等于没说）
