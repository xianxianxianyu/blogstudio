...
**不变量：** ① 单文档封闭（绝不读写 `docId` 以外的索引）；② 快照语义（贴入是深拷贝）；③ 纯被动无会话状态（历史由调用方传入）；④ 幂等索引（同一实例只建一次，`reindex()` 是唯一的失效入口；持久化落地后改为内容哈希）；⑤ 诚实性（`grounding` 独立于模型输出判定）；⑥ `ask` 只 reject 模型失败或契约违反，其余一律降级成合法 `Answer`。
