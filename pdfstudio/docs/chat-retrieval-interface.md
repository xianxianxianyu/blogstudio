...
**`grounding` 承诺什么（2026-08-14 定）：** `'retrieved'` 的含义是**检索到了主题相关的原文**，不是**答案确有原文支撑**。两者的差别由一条 eval 题逼出来：

> 问 DDPM「在 ImageNet 128×128 上的 FID 是多少」——全文 0 次提及 ImageNet，但满页都是 FID 表格。

检索找到 FID 表格是**对的**，那确实是全文最相关的段落。「这段里没有你问的那个事实」要把原文读一遍才知道——**那是推理，不是检索**，任何基于相似度的检索器都抓不到，换模型、调阈值、上 hybrid 都不行（实测：向量与关键词两路在这条题上同时失效）。

取更强的语义要付两种代价之一：加一次校验调用（与不变量⑤「grounding 独立于模型输出判定」冲突），或把这类判断塞进检索（做不到）。**选择较弱但诚实的语义**，并把判断权留给读者——`citations` 里有页码和原文片段，读者点开就能看到那是 FID 表而不是 ImageNet 数据。所以 UI 上 citation 必须显眼、可跳转，这是本决策的配套条件而非可选优化。

**取消语义：** 读者按下停止不是失败——`ask` 返回**已收到的那半段**作为合法 `Answer`（不变量 ⑥），不 reject。`signal` 透给 `ModelRequest`，adapter 据此断开远端流（ADR-0009）。

**不变量：** ① 单文档封闭（绝不读写 `docId` 以外的索引）；② 快照语义（贴入是深拷贝）；③ 纯被动无会话状态（历史由调用方传入）；④ 幂等索引（同一实例只建一次，`reindex()` 是唯一的失效入口；持久化落地后改为内容哈希）；⑤ 诚实性（`grounding` 独立于模型输出判定；`'retrieved'` 只承诺「检索到主题相关的原文」，见上）；⑥ `ask` 只 reject 模型失败或契约违反，其余一律降级成合法 `Answer`。
